...
import { createResource, deleteResource, listResources, renameResource, resourceFile, uploadResource, writeResource } from "./workspace-resources";
...
test("workspace resource operations stay inside the workspace", async () => {
  ...
  try {
    ...
    await writeResource(root, uploaded, "updated");
    expect(await readFile(join(root, uploaded), "utf8")).toBe("updated");
